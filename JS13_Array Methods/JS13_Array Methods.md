# Тема №13. Методы массивов 🧸

**Массивы предоставляют множество методов. Чтобы было проще, в этой теме они разбиты на группы!**

<div align="center">
  <img alt="Project Demo" src="./JS13.jpg" />
</div>
...
["Кринж", "Тильт", "Вайб"].forEach(alert);
```

А этот вдобавок расскажет и о позиции элемента в целевом массиве:

```js
["Кринж", "Тильт", "Вайб"].forEach((item, index, array) => {
...
### ☁️ `indexOf`/`lastIndexOf` и `includes`
...
```js 
let lengths = ["Рофл", "Кринж", "Вайб"].map(item => item.length);
alert(lengths); // 4,5,4
```
...
<div align="center">
  <img alt="Project Demo" src="./JS13_reduce.svg" />
</div>
...
### 🔹 Задача 2. Цифры числа в обратном порядке
...
### 🔹 Задача 3. Количество делителей числа
...
**Примечания**  
- Делители выводятся в виде массива, то есть в формате `[a, b, c]` или `[c]`, если число одно.  
- Для простых чисел (например, 13) выводится строка:  13 является простым

---

### 🔹 Задача 5. Горный массив
...
1. Первая функция проверяет, является ли массив `arr` горным с помощью **линейного поиска**.  
