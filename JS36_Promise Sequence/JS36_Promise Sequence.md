## Тема №36. Последовательность промисов 🍒

У нас есть последовательность асинхронных задач, которые должны быть выполнены одна за другой. Например, речь может идти о загрузке скриптов. Как же грамотно реализовать это в коде?

<div align="center">
  <img alt="Project Demo" src="./JS36.jpg" />
</div>

Промисы предоставляют несколько способов решения подобной задачи.

В этой теме мы разберём цепочку промисов.
...
<div align="center">
  <img alt="Project Demo" src="./JS36_promise-then-chain.svg" />
</div>
...
<div align="center">
  <img alt="Project Demo" src="./JS36_promise-then-many.svg" />
</div>
...
fetch('./user.json')
  ...
  .then(function(text) {
    ...
    alert(text); // {"name": "dv0retsky", isAdmin: true}
  });
```
...
fetch('user.json')
  ...
  .then(user => alert(user.name)); // dv0retsky, получили имя пользователя
```
...
<div align="center">
  <img alt="Project Demo" src="./JS36_promise-handler-variants.svg" />
</div>
