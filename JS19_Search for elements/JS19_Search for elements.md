...
<div align="center">
  <img alt="Project Demo" src="./JS19.jpg" />
</div>
...
- Находит элемент `<div>` и сохраняет в переменную `divElement`.
- Находит список `<ul>` и сохраняет в переменную `listElement`.
- Находит второй `<li>` (с текстом «Пит») и сохраняет в переменную `secondLiElement`.

Выведите все найденные элементы в консоль с помощью `console.log()` и убедитесь, что получены правильные элементы. При выполнении используйте методы навигации по DOM.
...
Создайте в HTML-документе таблицу для тестирования.
...
Напишите код, который выделит красным цветом все ячейки в таблице по обратной диагонали.

Нужно получить из таблицы `<table>` все диагональные `<td>` и выделить их, используя код:
...
Для приведённой ниже HTML-разметки напишите код, который находит следующие элементы и сохраняет их в отдельные переменные:

- Таблицу с `id="age-table"`.
- Все элементы `label` внутри этой таблицы (должно найтись 3 элемента).
- Первый `td` в этой таблице (с текстом `"Age"`).
- Форму с атрибутом `name="search"`.
- Первый `input` в этой форме.
- Последний `input` в этой форме.
- Выведите все найденные элементы в консоль и убедитесь в правильности поиска.
...
- Создайте заголовок `h1` с текстом `"Список пользователей"`.
- Создайте маркированный список `ul` с 3 элементами `li`: `"Анна"`, `"Борис"`, `"Виктор"`.
- Создайте кнопку `"Добавить пользователя"`.
- Реализуйте функционал: при клике на кнопку в список добавляется новый элемент `li` с текстом `"Новый пользователь"`.

Используйте методы `createElement()`, `appendChild()`, `textContent`.
...
- Находит все элементы с классом `text`.
- Для элемента с классом `special` устанавливает красный цвет текста.
- Для каждого третьего параграфа добавляет зелёный фон.
- Родительскому контейнеру добавляет рамку.
...
- Для активного элемента (с классом `"active"`) добавляет класс `"highlight"`.
- Вычисляет суммарную стоимость всех товаров на основе data-атрибутов.
- Находит товар с максимальной ценой и выводит его название в консоль.
...
Реализуйте функцию `findDomPath(startElement, targetElement)`, которая находит и возвращает CSS-путь между двумя элементами в DOM-дереве.

Путь должен быть в формате CSS-селектора, должны учитываться теги, классы и порядковые номера элементов.

Функция должна работать для любых двух элементов в DOM.
